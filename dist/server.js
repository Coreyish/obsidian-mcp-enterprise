...
export async function startServerStdio() {
    ...
    mcpServer.registerTool('write_note', {
    ...
    mcpServer.registerTool('list_notes', {
        ...
        return { content: [], structuredContent: { notes: notes.map(n => ({ ...n, modified: n.modified.toISOString() })) } };
    });
    ...
    mcpServer.registerTool('get_note_metadata', {
        ...
        return { content: [], structuredContent: { ...meta, modified: meta.modified.toISOString() } };
    });
    // Additional tools for better note management
    mcpServer.registerTool('update_note', {
        ...
        try {
            if (preserveFrontmatter) {
                ...
                return { content: [], structuredContent: { path: result.path, updated: true } };
            }
            else {
                const result = await vault.writeNote(path, content);
                return { content: [], structuredContent: { path: result.path, updated: true } };
            ...
            const result = await vault.writeNote(path, content);
            return { content: [], structuredContent: { path: result.path, updated: false } };
        }
    });
    mcpServer.registerTool('append_to_note', {
        ...
        try {
            ...
            return { content: [], structuredContent: { path: result.path, appended: true } };
        }
        catch (err) {
            // If note doesn't exist, create it
            const result = await vault.writeNote(path, content);
            return { content: [], structuredContent: { path: result.path, appended: false } };
        }
    });
    mcpServer.registerTool('delete_note', {
        ...
        try {
            ...
            return { content: [], structuredContent: { deleted: true, path: notePath } };
        }
        catch (err) {
            return { content: [], structuredContent: { deleted: false, path: notePath, error: 'Note not found or could not be deleted' } };
        }
    });
    mcpServer.registerTool('get_vault_stats', {
